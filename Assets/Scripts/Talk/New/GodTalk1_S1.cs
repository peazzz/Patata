...
public class GodTalk1_S1 : MonoBehaviour
{
    public static bool GT1_S1;
    public Text Godtext;
        ...
        if (Continue)
        {
            if (!loopOnce)
            {
                switch (nextMessage)
                {
                    ...
                    case 1:
                        textOver = false;
                        StartCoroutine(TypeWriterText(Godtext, "2ㄏ...", 0.1f));
                        break;
                    case 2:
                        textOver = false;
                        StartCoroutine(TypeWriterText(Godtext, "你是來求我讓你們復原嗎?", 0.1f));
                        break;
                    case 3:
                        textOver = false;
                        StartCoroutine(TypeWriterText(Godtext, "想要談條件的話...", 0.1f));
                        break;
                    case 4:
                        textOver = false;
                        StartCoroutine(TypeWriterText(Godtext, "就跟我來吧", 0.1f));
                        break;
                    case 5:
