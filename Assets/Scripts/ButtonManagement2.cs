	...
	void Update()
	{
		if (!GameStop._GameStop)
		{
			...
			if (btnn.Length > 0)
			{
				...
				for (int a = 0; a < btnn.Length; a++)
				{
					if (a == selected_Button)
					{
						btnn[a].GetComponent<Button>().Select();
						btnn[a].GetComponent<Image>().sprite = button2;
					}
					else
					{
						btnn[a].GetComponent<Image>().sprite = button1;
					}
				}

				//Get Position of all Buttons
				Vector3 but_pos = btnn[selected_Button].GetComponent<RectTransform>().position;

				//Array for differences in the positions of buttons
				float[] hor_dif = new float[btnn.Length];
				float[] vert_dif = new float[btnn.Length];
				...
				for (int a = 0; a < btnn.Length; a++)
				{
					if (a != selected_Button)
					{
						Vector3 but_pos2 = btnn[a].GetComponent<RectTransform>().position;
